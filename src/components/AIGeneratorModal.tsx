...
import { runFoundryIQGeneration } from '../services/foundryIQ';
...
export default function AIGeneratorModal({ isOpen, onClose, onSuccess }: Props) {
    ...
    const fetchRandomVideoBackground = async (folderPath: string) => {
        try {
            console.log('[foundry-iq] fetching random video background...');
            setStreamText(prev => prev + '\n\n[SYSTEM] Fetching random video background...\n\n');
            ...
            return new Promise<void>((resolve, reject) => {
                ...
                onEvent.onmessage = async (message) => {
                    if (videoFetched) return;
                    
                    if (message.items && message.items.length > 0) {
                        const videos = message.items.filter((i: any) => i.type === 'video' || i.media_type === 'video');
                        if (videos.length > 0) {
                            videoFetched = true;
                            ...
                            try {
                                ...
                                if (randomVideo.source === 'motionbgs' && randomVideo.detailUrl || randomVideo.detail_url) {
                                    const res: any = await invoke('resolve_motionbgs_video', { detailUrl: randomVideo.detailUrl || randomVideo.detail_url });
                                    if (res?.success && (res?.url4k || res?.url)) videoUrl = res.url4k || res.url;
                                } else if (randomVideo.source === 'wallpaperwaifu' && randomVideo.detailUrl || randomVideo.detail_url) {
                                    const res: any = await invoke('resolve_wallpaperwaifu_video', { detailUrl: randomVideo.detailUrl || randomVideo.detail_url });
                                    if (res?.success && res?.url) {
                                        const finalUrl = res.url4k || res.url;
                                        videoUrl = finalUrl.startsWith('/') ? `https://wallpaperwaifu.com${finalUrl}` : finalUrl;
                                        referer = randomVideo.detailUrl || randomVideo.detail_url;
                                    }
                                }
                                
                                if (videoUrl) {
                                    setStreamText(prev => prev + `[SYSTEM] Found video background From The Source Provider. Downloading...\n\n`);
                                    await invoke('download_ai_video_background', {
                                        folderPath,
                                        videoUrl,
                                        referer
                                    });
                                    setStreamText(prev => prev + `[SYSTEM] Background downloaded successfully..!\n\n`);
                                }
                            } catch (e) {
                                console.error('failed to resolve/download video:', e);
                            }
                            resolve();
                        }
                    }
                    if (message.isComplete && !videoFetched) {
                        resolve();
                    }
                };

                const queries = ['anime', 'snow', 'rain', 'cyberpunk', 'nature', 'space', 'city', 'lofi', 'neon', 'pixel art'];
                const randomQuery = queries[Math.floor(Math.random() * queries.length)];

                invoke('search_wallpapers', {
                    query: randomQuery,
                    sources: ['motionbgs', 'wallpaperwaifu'],
                    limitPerSource: 10,
                    randomize: true,
                    page: 1,
                    purity: '100',
            ...
            console.error('[foundry-iq] error fetching video:', error);
        }
    };
    ...
    const handleGenerate = async () => {
        ...
        while (attempts < maxAttempts && !success) {
            ...
            try {
                ...
                const fullResponse = await runFoundryIQGeneration(prompt, (token) => {
                ...
                setPhase('saving');
                const res: any = await invoke('save_ai_wallpaper', {
                    prompt,
                    html: cleanHtml,
                });

                if (res.success && res.message) {
                    setStreamText(prev => prev + '\n\n[SYSTEM] HTML saved. Mhmmm Initiating background download...\n\n');
                    await fetchRandomVideoBackground(res.message);
                    setPhase('done');
                    onSuccess(res.message);
                    onClose();
                    success = true;
                } else {
                    throw new Error(res.error || 'failed to save wallpaper');
                }
            } catch (err: any) {
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <span style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.01em' }}>
                            Generate Interactive Wallpaper
                        </span>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    <div style={{
                        ...
                        >
                            {isRunning ? (
                                ...
                                <><Wand2 size={16} /> Generate Wallpaper</>
                            )}
                        </button>
                    </div>

...
const ARCHIVE_README_URL: &str =
    "https://raw.githubusercontent.com/yapude/wallpapers/main/README.md";
...
pub async fn scrape_wallpaper_archive_laxentainc(
    query: &str,
    limit: usize,
    page: u32,
    ...
    let target_page = if query.is_empty() {
        ...
        println!(
            "[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] empty query - randomizing page to: {}",
            random_page
        );
        random_page
    } else {
    ...
    println!(
        "[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] starting scrape - query: '{}', page: {}, limit: {}",
    ...
    println!(
        "[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] fetched readme ({} bytes)",
        markdown.len()
    );
    ...
    println!(
        "[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] parsed {} total entries from readme",
        all_entries.len()
    );
        ...
        match TagCacheManager::new("tags.json") {
            Ok(cache) => {
                if let Err(e) = cache.add_tags(api_tags) {
                    println!("[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] Failed to cache tags: {}", e);
                }
            }
            Err(e) => println!("[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] Failed to init tag cache: {}", e),
        }
    }
    ...
    println!(
        "[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] {} entries match query '{}'",
        filtered.len(),
        query
    );
    ...
    let items: Vec<WallpaperItem> = page_entries
        ...
        .map(|(i, entry)| {
            ...
            let id = if id_slug.is_empty() {
                format!("laxentainc-{}-{}", target_page, i)
            } else {
                format!("laxentainc-{}", id_slug)
            };
            ...
            WallpaperItem {
                id,
                source: "wallpaper_archive_laxentainc".to_string(),
    ...
    if items.is_empty() {
        println!("[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] no items found");
        return Err("wallpaper archive returned no results".to_string());
    }

    println!(
        "[SCRAPER:WALLPAPER_ARCHIVE_LAXENTAINC] returning {} items",
        items.len()
    );
    Ok(items)
}
